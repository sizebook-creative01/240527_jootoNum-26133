...
$(function ($) {
  ...
  if (!query["yclid"] && !query["gclid"] && !query["utm_source"]) {
    //gclidもutm_sourceもクエリになければ、googleOptからリダイレクトされたものと判断し、クエリ付与
    work = decodeURIComponent(query["utm_referrer"])
      .replace("?", "&")
      .split("&");
  ...
  if (query["yclid"]) {
    ignore_yclid = 1;
    $.cookie("sizebook_yclid", query["yclid"], { expires: 1, path: "/" });
    suid_value = query["yclid"];
  } else if ($.cookie("sizebook_yclid")) {
    ignore_yclid = 1;
    suid_value = $.cookie("sizebook_yclid");
  } else if (query["gclid"]) {
    ignore_gclid = 1;
    $.cookie("sizebook_gclid", query["gclid"], { expires: 1, path: "/" });
    suid_value = query["gclid"];
  } else if ($.cookie("sizebook_gclid")) {
    // alert($.cookie('sizebook_gclid'));
    ignore_gclid = 1;
    suid_value = $.cookie("sizebook_gclid");
  } else if (query["utm_source"] == "yahoo") {
    utm_campaign_value = query["utm_campaign"];
    $.cookie("sizebook_utm_campaign", query["utm_campaign"], {
      expires: 1,
      path: "/",
    });
  ...
  function rename(atag) {
    ...
    try {
      ...
      atag.attr("href", lnk);
      if (lnk.hostname == "tr.threeate.jp")
        console.log("old=" + old + " new=" + atag.attr("href"));
